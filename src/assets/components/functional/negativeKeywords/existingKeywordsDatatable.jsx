...
const ExistingKeywordsDatatable = () => {
    ...
    const getKeywordsData = async () => {
        ...
        try {
            const url = `https://react-api-script.onrender.com/zydus/existing_negative_keyword?start_date=${startDate}&end_date=${endDate}&platform=${operator}`;
    ...
    const handleAddNegativeKeyword = async (row) => {
        ...
        try {
            const response = await fetch(`https://react-api-script.onrender.com/zydus/delete_negative_keyword?${params.toString()}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
