    ...
    const clearCampaignCaches = () => {
        try {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && key.includes('/zydus/campaign')) {
            ...
            renderCell: (params) => <BudgetCell
                ...
                onUpdate={async (campaignId, newBudget) => {
                    ...
                    try {
                        ...
                        let url = `https://react-api-script.onrender.com/zydus/campaign?start_date=${startDate}&end_date=${endDate}&platform=${operator}${ts}`;
        ...
        },
 {
    ...
    const handleCampaignClick = async (campaignName, campaignId) => {
        try {
            ...
            const startDate = formatDate(dateRange[0].startDate);
            const endDate = formatDate(dateRange[0].endDate);
            let url = `https://react-api-script.onrender.com/zydus/campaign_graph?start_date=${formatDate(startDate)}&end_date=${formatDate(endDate)}&platform=${operator}&campaign_id=${campaignId}`;
            if (selectedBrand && selectedBrand.trim() !== "") {
                url += `&brand_name=${encodeURIComponent(selectedBrand)}`;
            }
            const cacheKey = `cache:GET:${url}`;
    ...
    const confirmStatusChange = async (campaignId, newStatus, adType) => {
        try {
            ...
            const playPauseUrl = `https://react-api-script.onrender.com/zydus/play-pause?platform=${platformLower}`;
                    ...
                    <DialogActions>
                        ...
                        <Button color="primary" onClick={async () => {
                            ...
                            try {
                                ...
                                const response = await fetch("https://react-api-script.onrender.com/zydus/play-pause", {
                                    method: "PUT",
                                    headers: {
                                        "Content-Type": "application/json",
                                        Authorization: `Bearer ${token}`,
