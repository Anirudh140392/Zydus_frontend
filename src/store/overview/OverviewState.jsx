...
// Change: const [selectedBrand, setSelectedBrand] = useState(brandType || "zydus Cosmetics");
...
const OverviewState = (props) => {
    ...
    // Get selectedBrand from URL params - default to empty string instead of "zydus Cosmetics"
    ...
    const fetchAPI = async (endpoint, setLoadingState) => {
        ...
        try {
            let url = `${host}/zydus/new-overview?start_date=${startDate}&end_date=${endDate}&platform=${operator}`;
    ...
    const getOverviewData = useCallback(async (forceRefresh = false) => {
        ...
        let url = `${host}/zydus/new-overview?start_date=${startDate}&end_date=${endDate}&platform=${operator}${ts}`;
        // Only add brand_name parameter if a specific brand is selected (not empty string)
        if (selectedBrand && selectedBrand.trim() !== "") {
            url += `&brand_name=${encodeURIComponent(selectedBrand)}`;
        }
